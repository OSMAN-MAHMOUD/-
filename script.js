...
function grabHeader(){
  ...
  return {
    name: q('.t1-name, .t2-name, .t3-name, .t4-name, .t5-name, .t6-name, .t7-name, .t8-name'),
    role: q('.t1-role, .t2-role, .t3-role, .t5-role, .t6-role, .t7-role, .t8-role'),
    contacts: [...page.querySelectorAll('.t1-contact span, .t3-contact span, .t6-contact span, .sidebar .item, .sidebar .contact-text, .contact-line')].map(s=>s.innerHTML),
...
const templates = { 1:buildTpl1, 2:buildTpl2, 3:buildTpl3, 4:buildTpl4, 5:buildTpl5, 6:buildTpl6, 7:buildTpl7, 8:buildTpl8 };
const FLEX_PAGE_TPLS = [2,5];
